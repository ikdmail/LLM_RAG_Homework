# evaluate_answers.py (Gemini Flash評価 & 平均スコア版 - 外部JSONから正解読み込み)
...
# スクリプトが code/ ディレクトリにあることを前提とした相対パス。
...
# LLMへのAPI呼び出し間隔 (秒) - APIのレート制限回避のため、適切な間隔を設定。
API_CALL_DELAY = 1.0 # 例: 1秒間隔
...
def load_correct_answers(filepath: str) -> dict[str, str]:
    ...
    try:
        ...
        # 辞書のキー（質問）と値（回答）が文字列であることを確認する簡易チェック
...
def automated_evaluate(question: str, generated_answer: str, correct_answer: str, evaluator_llm_client, prompt_template: str) -> dict:
    """
    Uses an LLM to automatically evaluate a generated answer based on a specific prompt template.
    Returns a dictionary of scores and reasons, or an error dict.
    """
    ...
    try:
        # 評価者LLMを呼び出す
        # LLMが ```json ... ``` ブロックでJSONを出力することを期待
        response = evaluator_llm_client.generate_content(prompt)

        # レスポンステキストからJSONブロックを抽出してパース
        ...
        if not all(key in evaluation_result for key in expected_keys):
             print(f"Warning: Evaluator LLM returned unexpected JSON keys for question: {question[:50]}...")
             ...
             if key.endswith("_Score"):
                 try:
                     ...
                     if not (0 <= score <= 5):
                         ...
                         print(f"Warning: Evaluator LLM returned score out of 0-5 range ({score}) for question: {question[:50]}...")
                         score = max(0, min(5, score)) # スコアを0-5にクランプ
                     evaluation_result[key] = score
                 except (ValueError, TypeError):
                     # 数値に変換できない場合は警告を出し、スコアを0とする
                     print(f"Warning: Evaluator LLM returned non-integer score for question: {question[:50]}...")
                     evaluation_result[key] = 0 # 無効なスコアは0とする

        return evaluation_result

    except json.JSONDecodeError:
        print(f"Error: Failed to parse JSON from evaluator LLM response for question: {question[:50]}...")
        # print(f"Raw response text: {response.text[:500]}...") # 生のレスポンスの一部を表示（デバッグ用）
        return {"Error": "JSON parsing failed", "RawResponse": response.text if 'response' in locals() else 'N/A', "Prompt": prompt}
    except Exception as e:
        print(f"Error during automated evaluation for question: {question[:50]}...")
        print(f"Error details: {e}")
        return {"Error": f"Automated evaluation failed: {e}", "Prompt": prompt, "RawResponse": response.text if 'response' in locals() else 'N/A'}
...
if __name__ == "__main__":
    ...
    for i, question in enumerate(questions_to_evaluate):
        ...
        # print(f"正解: {correct_ans[:100]}...") # 正解は長い場合があるので表示省略
        # print(f"ベースLLM回答: {base_ans[:100]}...")
        # print(f"RAG回答: {rag_ans[:100]}...")
        ...
        if ENABLE_AUTOMATED_EVALUATION and evaluator_llm_client:
            ...
            for prompt_idx, prompt_template in enumerate(prompt_templates):
                ...
                # Base LLM回答の評価
                print("      -> BaseLLM...")
                base_eval_result = automated_evaluate(question, base_ans, correct_ans, evaluator_llm_client, prompt_template)
                if "Error" not in base_eval_result:
                    # 成功した場合、スコアをリストに追加
                    base_scores_list.append({
                        "Accuracy_Score": base_eval_result.get("Accuracy_Score", 0),
                        "Completeness_Score": base_eval_result.get("Completeness_Score", 0),
                        "Relevance_Score": base_eval_result.get("Relevance_Score", 0),
                    })
                    # 評価理由もリストに追加（短く表示）
                    base_reasons.append(f"P{prompt_idx+1}: Acc({str(base_eval_result.get('Accuracy_Reason', 'N/A'))[:50]}), Comp({str(base_eval_result.get('Completeness_Reason', 'N/A'))[:50]}), Rel({str(base_eval_result.get('Relevance_Reason', 'N/A'))[:50]})")
                    print("        -> Success.")
                else:
                    # 失敗した場合、理由リストにエラー情報を追加
                    print(f"        -> Failed (Error: {base_eval_result.get('Error', 'Unknown')}).")
                    base_reasons.append(f"P{prompt_idx+1}: Error - {base_eval_result.get('Error', 'Unknown')[:100]}") # エラーメッセージも短く

                # RAG回答の評価
                print("      -> RAG...")
                rag_eval_result = automated_evaluate(question, rag_ans, correct_ans, evaluator_llm_client, prompt_template)
                if "Error" not in rag_eval_result:
                     # 成功した場合、スコアをリストに追加
                     rag_scores_list.append({
                        "Accuracy_Score": rag_eval_result.get("Accuracy_Score", 0),
                        "Completeness_Score": rag_eval_result.get("Completeness_Score", 0),
                        "Relevance_Score": rag_eval_result.get("Relevance_Score", 0),
                    })
                     # 評価理由もリストに追加（短く表示）
                     rag_reasons.append(f"P{prompt_idx+1}: Acc({str(rag_eval_result.get('Accuracy_Reason', 'N/A'))[:50]}), Comp({str(rag_eval_result.get('Completeness_Reason', 'N/A'))[:50]}), Rel({str(rag_eval_result.get('Relevance_Reason', 'N/A'))[:50]})")
                     print("        -> Success.")
                else:
                     # 失敗した場合、理由リストにエラー情報を追加
                     print(f"        -> Failed (Error: {rag_eval_result.get('Error', 'Unknown')}).")
                     rag_reasons.append(f"P{prompt_idx+1}: Error - {rag_eval_result.get('Error', 'Unknown')[:100]}") # エラーメッセージも短く

                # --- API呼び出し間隔 ---
                # 短時間に何度もAPIを呼び出すとレート制限にかかる可能性があるため待機
                # 最後のプロンプの後 および 最後の質問以外に待機を入れる
                is_last_prompt = (prompt_idx == len(prompt_templates) - 1)
                is_last_question = (i == len(questions_to_evaluate) - 1)

                if not (is_last_question and is_last_prompt):
                     print(f"    -> API呼び出し間隔のため {API_CALL_DELAY} 秒待機...")
            ...
            if base_scores_list:
                ...
                result_entry["BaseLLM_Evaluation_Notes"] = f"Avg over {len(base_scores_list)}/{len(prompt_templates)} evals. Reasons: {' | '.join(base_reasons)}"
            else:
                 # 全ての評価が失敗した場合
                 result_entry["BaseLLM_Evaluation_Notes"] = f"Eval failed for all {len(prompt_templates)}. Reasons: {' | '.join(base_reasons)}"
            ...
            if rag_scores_list:
                ...
                # 注記に、成功した評価の回数と理由リストを格納
                result_entry["RAG_Evaluation_Notes"] = f"Avg over {len(rag_scores_list)}/{len(prompt_templates)} evals. Reasons: {' | '.join(rag_reasons)}"
            else:
                # 全ての評価が失敗した場合
                result_entry["RAG_Evaluation_Notes"] = f"Eval failed for all {len(prompt_templates)}. Reasons: {' | '.join(rag_reasons)}"
        ...
        # TODO: 必要なら手動評価をここに追加 (手動で評価結果を入力し、result_entryに加える)
        # manual_scores = manual_evaluate(question, base_ans, rag_ans, correct_ans)
        # result_entry.update(manual_scores) # 手動評価の結果を辞書に追加
    ...
    try:
        ...
        # Pandas DataFrame に変換してCSVとして保存
        df_results = pd.DataFrame(evaluation_results_list)
        # カラム順序を整理 (任意)
        # 評価注記カラムが長くなる可能性があるので最後に持っていくとCSVが見やすい
        column_order = [
            ...
            "BaseLLM_Evaluation_Notes", "RAG_Evaluation_Notes", # 注記を最後に
        ]
        # 実際のカラムリストに含まれるものだけを順序リストから取得
